...
import {Bell, Calendar, CheckCircle, Clock, Droplets, PawPrint, Shield, Thermometer, TrendingUp} from 'lucide-react';
...
import {AnimalPhase, getCountAnimalsPhases} from "@/lib/types/animal-phase.ts";
import {getWarnFromWeather} from "@/services/weather-service.ts";
import {WarnLevel} from "@/lib/types/wheater-warn.ts";
import {getPlaceholderFromTitleWeather} from "@/lib/types/wheater-type.ts";
...
const Dashboard = () => {
    ...
    const animalStats = getCountAnimalsPhases(animals);
    ...
    // Todos os alertas de clima
    const getWeatherAlert = () => {
        const weatherType = getWarnFromWeather(weather);
        if (!weatherType.warnLevel) return;
        return [{
            id: weatherType?.id,
            type: weather?.main,
            level: weatherType?.warnLevel,
            title: getPlaceholderFromTitleWeather(weather?.main),
            message: weatherType?.message,
                        ...
                        <CardContent>
                            ...
                            <div className="space-y-1 text-xs text-gray-600">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-medium">{animalStats[AnimalPhase.LACTACAO]}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span>🐮 Vacas Secas:</span>
                                    <span className="font-medium">{animalStats[AnimalPhase.VACA_SECA]}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span>🐂 Novilhas:</span>
                                    <span className="font-medium">{animalStats[AnimalPhase.NOVILHA]}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span>🐃 Bezerras:</span>
                                    <span className="font-medium">{animalStats[AnimalPhase.BEZERRA]}</span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                ...
                >
                    <Card className="bg-white shadow-md hover:shadow-lg transition-shadow">
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            ...
                            <Shield
                                className={`h-5 w-5 ${overdueVaccinations.length > 0 ? 'text-red-500' : 'text-green-500'}`}/>
                        </CardHeader>
                        <CardContent>
                            <div
                                className={`text-3xl font-bold ${overdueVaccinations.length > 0 ? 'text-red-600' : 'text-green-600'}`}>
                                ...
                                {allAlerts.map((alert) => (
                                    <div
                                        ...
                                        className={`p-3 rounded-lg border ${
                                            alert?.level === WarnLevel.CRITICAL
                            ...
                            {upcomingEvents.length === 0 ? (
                                ...
                                <div className="space-y-2">
                                    {upcomingEvents.slice(0, 4).map((event) => (
                                        <div key={event.id}
                                             className="flex items-center space-x-2 p-2 rounded bg-green-50">
                                    ...
                                    {upcomingEvents.length > 4 && (
                                        <p className="text-xs text-gray-500 text-center">
                                            +{upcomingEvents.length - 4} eventos
                                        </p>
                                    )}
                                </div>
                            )}
