...
const ImportAnimals = () => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Assumir formato CSV: tag,name,birth_date,phase
      const header = lines[0].toLowerCase().split(',').map(h => h.trim());
      const requiredColumns = ['tag', 'birth_date', 'phase'];
      ...
      const validPhases = ['bezerra', 'novilha', 'vaca_lactante', 'vaca_seca'];
      let successCount = 0;
      let failedCount = 0;
      const errors: string[] = [];
      ...
      for (let i = 1; i < lines.length; i++) {
        try {
          ...
          const name = nameIndex >= 0 ? data[nameIndex] || null : null;
          const birthDate = data[birthDateIndex];
          const phase = data[phaseIndex];

          // Validações
          if (!tag) {
            errors.push(`Linha ${i + 1}: Tag obrigatória`);
            failedCount++;
            continue;
          }

          if (!birthDate || !isValidDate(birthDate)) {
            errors.push(`Linha ${i + 1}: Data de nascimento inválida (use formato YYYY-MM-DD)`);
            failedCount++;
            continue;
          }

          if (!validPhases.includes(phase)) {
            errors.push(`Linha ${i + 1}: Fase inválida. Use: ${validPhases.join(', ')}`);
            failedCount++;
            continue;
          }

          // Criar animal (simulação - aqui você chamaria a API real)
          console.log('Criando animal:', { tag, name, birth_date: birthDate, phase });
      ...
      if (successCount > 0) {
        toast.success(`${successCount} animais importados com sucesso!`);
        refetch(); // Recarregar a lista de animais
      }
  ...
  const downloadTemplate = () => {
    const csvContent = 'tag,name,birth_date,phase\n001,"Vaca Mimosa",2020-03-15,vaca_lactante\n002,"Bezerra Luna",2023-08-20,bezerra';
    const blob = new Blob([csvContent], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'template_animais.csv';
    a.click();
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>Importar Animais</span>
        </CardTitle>
        <CardDescription>
          Importe animais em lote através de arquivo CSV ou Excel
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <Alert>
          ...
          <AlertDescription>
            <strong>Formato esperado:</strong> O arquivo deve conter as colunas: tag, name (opcional), birth_date (YYYY-MM-DD), phase
            <br />
            <strong>Fases válidas:</strong> bezerra, novilha, vaca_lactante, vaca_seca
          </AlertDescription>
        </Alert>
